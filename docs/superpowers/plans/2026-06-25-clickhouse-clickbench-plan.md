...
정적 바인딩(사용자 지정 snap-024, 고유 이름 `clickhouse-hits-024`):
VolumeSnapshotContent `clickhouse-hits-024-content`(`source.snapshotHandle: snap-024c86faa00cd0448`,
`driver: ebs.csi.aws.com`, `volumeSnapshotClassName: gp3-clickhouse-snapclass`, `deletionPolicy: Retain`,
`volumeSnapshotRef: {name: clickhouse-hits-024, namespace: benchmark}`) +
VolumeSnapshot `clickhouse-hits-024`(`source.volumeSnapshotContentName: clickhouse-hits-024-content`).

**라이브 충돌 없음**: 기존 `clickhouse-hits-snap`(snap-057)과 이름이 다르므로 **live apply 안전**.
Retain이라 삭제해도 원본 snap-024 보존. Job PVC dataSource가 이 이름을 참조(정합 필수).

- [ ] VolumeSnapshotContent(volumeSnapshotRef) + VolumeSnapshot(source.volumeSnapshotContentName) — 고유 이름 clickhouse-hits-024
- [ ] validate.sh에 YAML 파싱 + Job PVC dataSource 이름 정합 검증 추가
...
다중 문서 YAML: PVC(dataSource VolumeSnapshot `clickhouse-hits-024`, SC `gp3-clickhouse`, 100Gi, RWO) + Job.
