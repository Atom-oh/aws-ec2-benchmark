...
hits self-JOIN 대표 집계 쿼리. 8GB 인스턴스 OOM 방지 위해
`SETTINGS join_algorithm='grace_hash', max_bytes_before_external_group_by=<n>` 적용.

- [ ] self-JOIN 쿼리 작성 (grace_hash + spill 설정)
- [ ] validate.sh에 구문 sanity + SETTINGS 존재 검증 추가
...
- **메인 컨테이너**(이미지 `clickhouse/clickhouse-server:CLICKHOUSE_VERSION`): 실행 스크립트가
  ① server 백그라운드 기동 → ② ready 폴링 → ③ 쿼리 **개별 실행**(per-query cold/hot 타이밍, §10 로그 포맷) →
...
- [ ] PVC + Job + initContainer + 실행 스크립트(백그라운드 기동/ready 대기/개별 쿼리/정상 종료) 작성
- [ ] ConfigMap 마운트, podAntiAffinity·toleration·nodeSelector·backoffLimit 0 포함
