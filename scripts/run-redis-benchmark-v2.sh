#!/bin/bash
# Redis Benchmark Runner v2
# Efficient: Server+Benchmark pair deployment with immediate cleanup
# Based on nginx-benchmark.sh pattern

cd /home/ec2-user/benchmark

SERVER_TEMPLATE="benchmarks/redis/redis-server.yaml"
BENCHMARK_TEMPLATE="benchmarks/redis/redis-benchmark.yaml"
...
collect_and_cleanup() {
  ...
  kubectl delete deployment -n benchmark "redis-server-${safe}" --ignore-not-found=true &>/dev/null
  kubectl delete service -n benchmark "redis-server-${safe}" --ignore-not-found=true &>/dev/null
...
echo "===== Redis Benchmark Runner v2 ====="
echo "Run: $RUN"
