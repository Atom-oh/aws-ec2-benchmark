#!/bin/bash
# Nginx Benchmark Runner
# Max 20 concurrent jobs, auto-collect results and delete completed jobs

BENCHMARK_TEMPLATE="/home/ec2-user/benchmark/benchmarks/nginx/nginx-benchmark.yaml"
SERVER_TEMPLATE="/home/ec2-user/benchmark/benchmarks/nginx/nginx-server.yaml"
...
check_server_exists() {
  ...
  kubectl get deployment -n benchmark "nginx-server-${safe_name}" &>/dev/null
}
...
deploy_server() {
  ...
  echo "  [DEPLOY] nginx-server-${safe_name}"
...
start_benchmark() {
  ...
  ready=$(kubectl get pods -n benchmark -l app=nginx-server,instance-type="${instance}" -o jsonpath='{.items[0].status.containerStatuses[0].ready}' 2>/dev/null)
  if [ "$ready" != "true" ]; then
...
collect_and_delete() {
  local instance=$1
  local safe_name=$(echo "$instance" | tr '.' '-')
  local job_name="nginx-benchmark-${safe_name}"
...
while true; do
  ...
  for instance in "${!STARTED[@]}"; do
    ...
    job_name="nginx-benchmark-${safe_name}"
