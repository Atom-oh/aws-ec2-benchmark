#!/bin/bash
# Spring Boot Benchmark Runner v2
...
BENCHMARK_TEMPLATE="benchmarks/springboot/springboot-benchmark.yaml"
...
ensure_service() {
  ...
  if ! kubectl get svc -n benchmark "springboot-server-${safe}" &>/dev/null; then
    # Create service manually
    cat <<EOF | kubectl apply -f - 2>/dev/null
apiVersion: v1
kind: Service
metadata:
  name: springboot-server-${safe}
  namespace: benchmark
  labels:
...
is_server_ready() {
  local instance=$1
  local ready=$(kubectl get pods -n benchmark -l "app=springboot-server,instance-type=${instance}" -o jsonpath='{.items[0].status.containerStatuses[0].ready}' 2>/dev/null)
  [ "$ready" = "true" ]
}

# Deploy benchmark job (wrk part only)
deploy_benchmark() {
  local instance=$1
  local run=$2
  local safe=$(echo "$instance" | tr '.' '-')
  local jobname="springboot-benchmark-${safe}-run${run}"

  # Extract only the second Job (springboot-benchmark-*) from the template
  sed -e "s/springboot-benchmark-INSTANCE_SAFE/${jobname}/g" \
      -e "s/INSTANCE_SAFE/${safe}/g" \
      -e "s/\${INSTANCE_TYPE}/${instance}/g" \
      "$BENCHMARK_TEMPLATE" | awk '/^---$/,0' | tail -n +2 | kubectl apply -f - 2>/dev/null
}
...
collect_and_cleanup() {
  ...
  kubectl delete deployment -n benchmark "springboot-server-${safe}" --ignore-not-found=true &>/dev/null
  kubectl delete service -n benchmark "springboot-server-${safe}" --ignore-not-found=true &>/dev/null
...
echo "===== Spring Boot Benchmark Runner v2 ====="
echo "Run: $RUN"
